...
export default function Group() {
    ...
    fetchGroups()
      .then((response) => {
        ...
        setGroupsWithPersonCount(groupsData.map(group => ({
          id: group.id,
          name: group.name,
          personCount: group.persons ? group.persons.size : 0,
          tipo: group.tipoGrupo ? group.tipoGrupo.name : '' // Asegúrate de que 'tipoGrupo' esté presente en los datos
        })))
      })
      .catch((error) => {
        console.log(error);
  ...
  return (
    ...
    >
      ...
      {/* Define la cuarta columna con el campo 'tipo' y un título personalizado 'Tipo de grupo'. */}
      <Column dataField="tipo" caption="Tipo de grupo" />
    </DataGrid>
  );
}